		...
		int include_empty)
{
	...
	while (src_string && *src_string) {
		...
		if (src_string == p) {
			if (include_empty) {
				arg[0] = '\0';
				if (RT_SUCCESS != argv_append(&argc, &argv, arg))
					return NULL;
			}
		}
		...
		/* tail argument, add straight from the original string */
			...
			if (RT_SUCCESS != argv_append(&argc, &argv, src_string))
				return NULL;
			src_string = p;
			continue;
		}
		...
		/* long argument, malloc buffer, copy and add */
			...
			if (RT_SUCCESS != argv_append(&argc, &argv, argtemp)) {
				free(argtemp);
				return NULL;
			}
...
char **argv_copy(char **argv)
{
	...
	while (NULL != *argv) {
		if (RT_SUCCESS != argv_append(&dupc, &dupv, *argv)) {
			argv_free(dupv);
			return NULL;
		}
